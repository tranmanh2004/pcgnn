"""Smoke test: intra_distance = bc_distance + astar_edit_diversity"""
...
ME_PATH_BINS   = [0.5, 1.0, 1.5]
ME_WALL_BINS   = [0.2, 0.4, 0.6]
ME_ROWS = len(ME_PATH_BINS) + 1
ME_COLS = len(ME_WALL_BINS) + 1
ME_TOTAL_CELLS = ME_ROWS * ME_COLS

random.seed(SEED); np.random.seed(SEED)


def generate_level(net, map_h=14, map_w=14):
...
def astar_difficulty(level):
    path, nc = _get_astar_result(level)
    if path is None: return 0.0
    return max(nc-len(path),0)/max(1,int((level!=WALL).sum())-len(path))

def behavior_characterization(level):
    h,w = level.shape; walkable = {FLOOR,PLAYER,ENEMY}
    ...
    path,_=_get_astar_result(level); turns_norm=0.0
    ...
    return np.array([wall_dens,path_norm,dead/total,branch/total,min(1.0,regions/10.0),min(1.0,astar_difficulty(level)),turns_norm])
...
def astar_edit_diversity(level_a, level_b) -> float:
    path_a,_ = _get_astar_result(level_a); path_b,_ = _get_astar_result(level_b)
    if not path_a: path_a = [(0,0)]
    if not path_b: path_b = [(0,0)]
    sa = _path_to_actions(path_a); sb = _path_to_actions(path_b)
...
def _aed_cached(level_a, level_b) -> float:
    ...
    if key not in _aed_cache: _aed_cache[key]=astar_edit_diversity(level_a,level_b)
    return _aed_cache[key]

def intra_distance(level_a, level_b, alpha=0.6) -> float:
    return alpha*bc_distance(level_a,level_b) + (1.0-alpha)*_aed_cached(level_a,level_b)
...
def intra_novelty_score(levels, k=2):
    ...
    for i,li in enumerate(levels):
        dists=sorted(intra_distance(li,lj) for j,lj in enumerate(levels) if j!=i)
...
def _me_cell(level):
    ...
    return int(np.searchsorted(ME_PATH_BINS,path_norm)), int(np.searchsorted(ME_WALL_BINS,wall))

def me_genome_coverage(levels):
    return len(set(_me_cell(l) for l in levels))/ME_TOTAL_CELLS
...
def eval_genomes(genomes, config):
    ...
    for gid,genome in genomes:
        ...
        genome.fitness=(NOVELTY_W_SOLVABLE*f_solve+NOVELTY_W_INTRA*f_intra+
                        NOVELTY_W_INTER*f_inter+NOVELTY_W_ME*f_me)
...
class Reporter(neat.reporting.BaseReporter):
    def post_evaluate(self,config,population,species,best):
        ...
        me_refs=[levels for (_,levels) in me_archive.values()]
        inter=inter_novelty_score(sample,me_refs+novelty_archive[-50:]) if (me_refs or novelty_archive) else 0.0
        cov=me_genome_coverage(sample)
        g=len(gen_log); gen_log.append({})
        print(f"  gen {g:2d}/{MAX_GEN}  best={max(fits):.3f}  sol={sol:.2f}"
              f"  intra={intra:.3f}  inter={inter:.3f}  ME:{len(me_archive)}/{ME_TOTAL_CELLS}  cov={cov:.2f}",flush=True)

pop=neat.Population(config); pop.add_reporter(Reporter())
t0=time.time()
winner=pop.run(eval_genomes,MAX_GEN)
elapsed=time.time()-t0

print(f"\nDone in {elapsed:.0f}s  (cache hits: aed={len(_aed_cache)}, bc={len(_bc_cache)})")
...
print("\nMAP-Elites grid:")
path_labels=["path<.5",".5-1.0","1.0-1.5",">=1.5"]
print("         wall<.2  .2-.4  .4-.6  >=.6")
for r in range(ME_ROWS):
    row_str=f"{path_labels[r]:8s} "
    for c in range(ME_COLS): row_str+="  [X]" if (r,c) in me_archive else "  [ ]"
    print(row_str)

chars={WALL:"#",FLOOR:".",PLAYER:"P",ENEMY:"E"}
print("\nSample maps (3):")
for lvl in sample[:3]:
    for row in lvl: print("".join(chars.get(t,"?") for t in row))
    print()
