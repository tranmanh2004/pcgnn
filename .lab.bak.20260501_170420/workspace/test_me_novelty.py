"""Smoke test: MAP-Elites (path_len x wall_dens) + novelty"""
...
class Reporter(neat.reporting.BaseReporter):
    def post_evaluate(self, config, population, species, best):
        ...
        me_refs  = [levels for (_, levels) in me_archive.values()]
        all_refs = me_refs + novelty_archive[-50:]
        inter = inter_novelty_score(sample, all_refs) if all_refs else 0.0
        cov   = me_genome_coverage(sample)
        g = len(gen_log)
        gen_log.append(dict(g=g, best=max(fits), sol=sol, intra=intra, inter=inter, cov=cov))
...
print(f"  solve={solvability_fitness(sample):.2f}  intra={intra_novelty_score(sample,k=5):.3f}  ME={len(me_archive)}/{ME_TOTAL_CELLS}")
print(f"  Winner: nodes={len(winner.nodes)}, conns={len(winner.connections)}")

# Show ME grid occupancy
...
for r in range(ME_ROWS):
    ...
    for c in range(ME_COLS):
        row_str += "  [X]" if (r,c) in me_archive else "  [ ]"
    print(row_str)
